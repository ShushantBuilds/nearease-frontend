...
export default function MyBookings() {
  ...
  const printBill = (booking) => {
    // A simple, clean way to print a receipt using the browser's native print engine
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <div class="row"><strong>Date:</strong> <span>${new Date(booking.scheduleTime).toLocaleString()}</span></div>
          <div class="row"><strong>Service:</strong> <span>${booking.serviceOffering?.name || 'Service'}</span></div>
          ...
          <div class="row total"><strong>Total Paid:</strong> <span>₹${booking.serviceOffering?.price || booking.price || 0}</span></div>
        </body>
      </html>
    `);
        ...
        <div className="space-y-6">
          {bookings.map((booking) => (
            <div key={booking.id} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden transition-all hover:shadow-md">
              ...
              {/* STATUS BANNERS - Tailored to your exact flow */}
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">{booking.serviceOffering?.name || "Service Booking"}</h3>
                  ...
                  <div className="text-right">
                     ...
                     <p className="text-2xl font-black text-indigo-600 dark:text-indigo-400">₹{booking.serviceOffering?.price || booking.price || 0}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-xl border border-gray-100 dark:border-gray-700 mb-4">
                  <p className="text-gray-600 dark:text-gray-300 flex items-start gap-2">
                    ...
                    <span><span className="block font-semibold text-gray-900 dark:text-gray-100">Scheduled Time</span>{new Date(booking.scheduleTime).toLocaleString()}</span>
                  </p>
                  <p className="text-gray-600 dark:text-gray-300 flex items-start gap-2">
